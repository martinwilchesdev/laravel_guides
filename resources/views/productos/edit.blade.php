...
<div class="mx-w-lg mx-auto bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="{{ route('productos.update', $producto->id) }}" method="POST">
        ...
        <input type="text" name="nombre" class="w-full p-2 border-b-2 mb-3 outline-none" autocomplete="off" value="{{ $producto->nombre }}" required><br>

        <label class="block mb-2 font-semibold" for="precio">Precio:</label>
        <input type="number" name="precio" step="0.01" class="w-full p-2 border-b-2 mb-3 outline-none" value="{{ $producto->precio }}" required><br>

        <label class="block mb-2 font-semibold" for="descripcion">Descripcion:</label>
        <textarea name="descripcion" class="bg-blue-400 px-4 py-2 w-full outline-none resize-none">{{ $producto->descripcion }}</textarea><br>

        <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded-lg cursor-pointer mt-4">Actualizar</button>
    </form>
</div>
@endsection
